...
namespace Repositories
{
	public class OrderRepository : IOrderRepository, IDisposable
	{
		...
		public async Task<IEnumerable<Order>> GetAllIn24Hours()
		{
			string sqlGetAllOrdersIn24Hours = "SELECT * FROM Orders WHERE CreatedAt >= DATEADD(HOUR, -24, GETDATE())";
			string sqlGetOrderItemsByOrderIds = "SELECT * FROM OrderItems WHERE OrderId IN ({0})";
			string sqlGetReservationsByIds = "SELECT * FROM Reservations WHERE ResId IN ({0})";
			string sqlGetTablesByIds = "SELECT * FROM Tables WHERE TableId IN ({0})";
			string sqlGetDishesByIds = "SELECT * FROM Dishes WHERE DishId IN ({0})";

			// Get all orders in 24 hours
			var orders = await _context.Orders.FromSqlRaw(sqlGetAllOrdersIn24Hours).ToListAsync();
			...
			if (reservationIds.Count != 0)
			{
				...
				foreach (var order in orders)
				{
					if (order.ResId.HasValue && reservations.TryGetValue(order.ResId.Value, out var reservation))
					{
						order.Reservation = reservation;
					}
				}
			...
			if (tableIds.Count != 0)
			{
				...
				foreach (var order in orders)
				{
					if (order.TableId.HasValue && tables.TryGetValue(order.TableId.Value, out var table))
					{
						order.Table = table;
					}
				}
			...
			// Include Dishes
			var dishIds = orders.SelectMany(o => o.OrderItems.Select(oi => oi.DishId)).Distinct().ToList();
