...
namespace Repositories
{
	public class StorageRepository : IStorageRepository, IDisposable
	{
		...
		{
			string sqlUpdateStorage = "UPDATE Storages SET ItemName = {0}, Unit = {1}, Quantity = {2} WHERE ItemId = {3}";

			await _context.Database.ExecuteSqlRawAsync(sqlUpdateStorage, storage.ItemName, storage.Unit, storage.Quantity, storage.ItemId);
		}
